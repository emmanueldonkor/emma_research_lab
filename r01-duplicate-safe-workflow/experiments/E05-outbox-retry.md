...
On 2026-09-08, `retryable-notification` created one outbox message. The first dispatch used the controlled failure mode; the target message remained pending with `deliveryAttempts = 1` and an error. The next dispatch marked it published with `deliveryAttempts = 2` and no remaining error.
